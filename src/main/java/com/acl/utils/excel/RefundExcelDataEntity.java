...
public class RefundExcelDataEntity {
    /**
     * 以及标题
     */
    private static Map<String, ParsingShuaBaoMExcelUtil.MergedRowTitle> mapTitle = new HashMap<String, ParsingShuaBaoMExcelUtil.MergedRowTitle>();

    public  BasicExcelDataEntity getParsingExcelResult(File file){
        Map<String,String> titleIndex = new HashMap<>();
        Map<String,String> titleResult = this.readExcelToObjIndex(file,0);
        ...
        ArrayList<Map<String,String>> result = this.readExcelToObj(file);
...
//        }
    ...
    private Map<String,String> readExcelIndex(Workbook wb, int sheetIndex, int startReadLine, int tailLine) {
        ...
        for(Cell c : row) {
            ...
            if(isMerge) {
                ...
                returnStr = rs;
                ParsingShuaBaoMExcelUtil.MergedRowTitle excelTitle = getMergedRowExcelTitle(sheet, startReadLine, c.getColumnIndex());
                if(excelTitle!=null){
     ...
     * @return
     */
    private ParsingShuaBaoMExcelUtil.MergedRowTitle getMergedRowExcelTitle(Sheet sheet, int row , int column) {
        int sheetMergeCount = sheet.getNumMergedRegions();
        for (int i = 0; i < sheetMergeCount; i++) {
            ...
            if(row == firstRow && row == lastRow){
                if(column >= firstColumn && column <= lastColumn){
                    ...
                    ParsingShuaBaoMExcelUtil.MergedRowTitle excelTitle = new ParsingShuaBaoMExcelUtil.MergedRowTitle();
    ...
    public static void main(String[] args){
        RefundExcelDataEntity excelUtil = new RefundExcelDataEntity();
        //读取excel数据
