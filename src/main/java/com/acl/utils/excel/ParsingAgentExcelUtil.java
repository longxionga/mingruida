...
import com.acl.platform.controller.PlatformAgentUserController;
 ...
 */
public class ParsingAgentExcelUtil {
    private static final Logger LOGGER = LoggerFactory.getLogger(ParsingAgentExcelUtil.class);
...
//        }
        ...
        for(Map<String,String> map:result){
            ...
            for (String str:titleResult.keySet()){
                ...
                if( titleIndex.containsKey(colName) && colName.equals("所属渠道")){
                    basicInfo.setChName(map.get(str));
                    continue;
                }
                if( titleIndex.containsKey(colName) && colName.equals("后台名称")){
                    basicInfo.setBackName(map.get(str));
                    continue;
                }
                if( titleIndex.containsKey(colName) && colName.equals("代理名称")){
                    basicInfo.setAgentName(map.get(str));
                    continue;
                }
                ...
                if((titleIndex.containsKey("薪资及绩效提成") &&  colName.equals("薪资及绩效提成"))  || (titleIndex.containsKey("项目明细") &&  colName.equals("项目明细"))){
                    ...
                    for (String keys:strList){
                        if(keys.equals(str)){
                            ...
                            //LOGGER.info(colName+"，从行数据中解析的名字："+titleRowResult.get(keys)+",key:"+keys+",value:"+map.get(keys));
                ...
                if((titleIndex.containsKey("应扣工资") &&  colName.equals("应扣工资"))  || (titleIndex.containsKey("应扣款项") &&  colName.equals("应扣款项"))){
                    List<String> strList = Arrays.asList(StringUtils.split(titleIndex.get(colName),","));
                    Collections.sort(strList);
                    ...
                    for (String keys:strList){
                        if(keys.equals(str)){
                            ...
                            mergedRowTitle.setColumnName(titleResult.get(keys));
                            mergedRowTitle.setCurrentColumnName(titleRowResult.get(keys));
                            //System.out.println(colName+"，从行数据中解析的名字："+titleRowResult.get(keys)+",key:"+keys+",value:"+map.get(keys));
                            mapStrs.put(keys,mergedRowTitle);
                ...
                if((titleIndex.containsKey(colName) && colName.equals("预支工资")) || (titleIndex.containsKey(colName) && colName.equals("抵扣所得分润"))){
                    basicInfo.setDeductAmount(map.get(str));
                    continue;
                }

                if((titleIndex.containsKey(colName) && colName.equals("实发工资")) || (titleIndex.containsKey(colName) && colName.equals("实发分润"))){
                    basicInfo.setRealAmount(map.get(str));
                    continue;
                }

                if(titleIndex.containsKey(colName) && colName.equals("品牌")){
                    basicInfo.setBrand(map.get(str));
                    continue;
                }
    ...
    public static void main(String[] args){
        ParsingAgentExcelUtil excelUtil = new ParsingAgentExcelUtil();
        //读取excel数据
        String path = "D:\\Maoda\\查看工资分润程序需求\\查看工资分润程序需求\\代理商分润模板.xlsx";
        ...
        for(String s:titleRowResult.keySet()){
            ...
            if(mergedRowTitle!=null){
                // System.out.println("交叉比对索引："+s+",值:"+titleRowResult.get(s).toString()+" ,合并名称："+mergedRowTitle.getColumnName());
            }else{
                //System.out.println("交叉比对索引："+s+",值:"+titleRowResult.get(s).toString()+" ,存在合并"+mapTitle.containsKey(s));
            }
        }
        for(String s:mapTitle.keySet()){
            // System.out.println("合并的 key : "+s+" value : "+mapTitle.get(s).toString());
        }
        ...
        for(Map<String,String> map:result){
            ...
            for(String s:map.keySet()){
               /* String indexValue = titleIndex.get("序号");
                ...
                String indexValue = titleIndex.get("手机号");
                if(indexValue!=null){
