	...
	$('#data').datagrid(
					{
		                ...
		                onExpandRow: function(index,row){
		                    ...
		                    ddv.datagrid({
		                        url:"queryReportDeptDataMonthInfo?year="+ row.FOR_YEAR +"&month="+ row.FOR_MONTH +"&settle_id="+row.SETTLE_ID+"&agent_id="+ row.AGENT_ID,
		                        fitColumns:true,
		                        singleSelect:true,
		                        rownumbers:true,
		                        height:'auto',
...
$("#search").submit(function() {
	...
	loadData(param,"queryReportDataMonthInfo");//查询数据
	counterfeesum(param);
	return false;
});
...
function viewdetail(settle_id,agent_id,dept_id,year,month,day){
	$("#data2").html("");
	$('#txsh_detail_2').dialog('open');
	var data = eval("({settle_id:'"+settle_id+"',agent_id:'"+agent_id+"',dept_id:'"+dept_id+"',year:'"+year+"',month:'"+month+"'})")
	LoadDetailInfo(data);
}
