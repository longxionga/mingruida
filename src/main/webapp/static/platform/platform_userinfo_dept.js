	...
	var  columns  =  [
		...
		if(dept_type==0){
			columns.push({
	  	   ...
	  	   align: 'center',
	  	   formatter: function (value, row, index) {
	  		 var str='\<a class="btns" href="javascript:queryBrokerUser(\''+ index+ '\');">变更公司代理商</a>\
	           ';  
	  		   if(row.is_use==2){
...
function loadAgentDept(param) {
	$('#broker').datagrid({
		...
		pageSize : 20,// 每页显示数量 参数必须为pageList的对应
		pageList : [ 20,40,80 ],// 每页显示数量设置参数
		url: "queryDeptInfoByType?dept_type=4",
		queryParams: param,//查询参数
						...
						},
		           {
		        	   ...
		        	   formatter: function (value, row, index) {
		        		   if(row.is_use==0){
		        			   ...
		        			   var str = '\
			        			   <a class="btns" href="javascript:getSelected(\''+ row.agent_id+ '\');">更换</a>\
									';
			        			   return str;
		        		   }
		        	   }}
