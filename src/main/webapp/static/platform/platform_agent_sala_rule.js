	...
	$('#data').datagrid({
		...
		columns: [[
					...
					},
		           {
		        	   ...
		        	   formatter: function (value, row, index) {
		        		   var str = '\
		        		      <a class="btns" href="javascript:upFiles(\'' + row.id + '\');">上传工资</a>\
		        			   <a class="btns" href="javascript:update(\'' + index + '\');">编辑</a>\
		        			   <a class="btns" href="javascript:off(\'' + index + '\');">删除代理商工资</a>\
		        			   ';
		        		   return str;
		        	   }
		           }
...
function off(id) {
    $.messager.confirm('删除代理商数据', '是否删除代理商数据', function(r) {
        if (r) {
