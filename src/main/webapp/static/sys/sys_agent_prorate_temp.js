	...
	$('#data').datagrid(
					{
						...
						columns : [ [
								...
								},
								{
									...
									formatter : function(value, row, index) {
										if($('#deptType').val()<=4){
											return '<font color=red>'+row.b_allot_1+'%</font> '+"比例1";
										}else{
											return row.a_name;
										}
									}
								},
								{
									...
									formatter : function(value, row, index) {
										if($('#deptType').val()<=4){
											return '<font color=red>'+row.b_allot_2+'%</font> '+"比例2";
										}else{
											return row.a_name;
										}
									}
								},
								{
									...
									formatter : function(value, row, index) {
										if($('#deptType').val()<=4){
											return '<font color=red>'+row.b_allot_3+'%</font> '+"比例3";
										}else{
											return row.a_name;
										}
									}
...
function update(id) {
    ...
    $('#allot_1').html(" <font color=red>"+$('#data').datagrid('getData').rows[id].b_allot_1+"</font>");
    $('#allot_2').html(" <font color=red>"+$('#data').datagrid('getData').rows[id].b_allot_2+"</font>");
    $('#allot_3').html(" <font color=red>"+$('#data').datagrid('getData').rows[id].b_allot_3+"</font>");
	//$("#a_allot").textbox("disableValidation").textbox("readonly");
    //$("#ce_allot").textbox("disableValidation").textbox("readonly");
    //$("#ch_allot").textbox("disableValidation").textbox("readonly");
    //$("#s_allot").textbox("disableValidation").textbox("readonly");
...
$("#updateform").submit(function() {
	...
	var vv = 100-$('#ce_allot').textbox('getValue')-
		...
		$('#b_allot_1').textbox('getValue')-
        $('#b_allot_2').textbox('getValue')-
    	$('#b_allot_3').textbox('getValue');
	// if($('#s_allot').textbox('getValue') <40)
	// {
	// 	$.messager.alert('错误信息',"服务商比例不能小于40%",'error');
	// 	return false;
	// }
	// if($('#b_allot_1').textbox('getValue') >20)
	// {
	// 	$.messager.alert('错误信息',"比例1不能大于20%",'error');
	// 	return false;
	// }
    // if($('#b_allot_2').textbox('getValue') >20)
    // {
     //    $.messager.alert('错误信息',"比例2不能大于20%",'error');
     //    return false;
    // }
    // if($('#b_allot_3').textbox('getValue') >20)
    // {
     //    $.messager.alert('错误信息',"比例3不能大于20%",'error');
     //    return false;
    // }
	...
	// }
	if(vv!=0){
		$.messager.alert('错误信息',"请重新验算分配比率,分配比例需等于100%",'error');
		return false;
	}
});
